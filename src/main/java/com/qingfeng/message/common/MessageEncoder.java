...
public class MessageEncoder  extends MessageToByteEncoder<Message> {
    ...
    protected void encode(ChannelHandlerContext ctx, Message message, ByteBuf out) {
        ...
        if (message.getHead().getMessageType() != MessageTypeEnum.EMPTY) {
            ...
            if (message.getHead().getSessionId() ==null || "".equals(message.getHead().getSessionId())) {
                // 生成一个sessionId，并将其写入到字节序列中
                String sessionId = UUID.randomUUID().toString();
                message.getHead().setSessionId(sessionId);
                out.writeCharSequence(sessionId, Charset.defaultCharset());
            }

            out.writeByte(message.getHead().getMessageType().getType());// 写入当前消息的类型
